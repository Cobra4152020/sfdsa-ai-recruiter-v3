...
export async function fixAdminProfile(userId: string) {
  try {
    ...
    if (userTypeError && userTypeError.code !== "PGRST116") {
      return {
        success: false,
        error: `Error checking user type: ${userTypeError.message}`,
      };
    }

    // 3. Insert or update user_types if needed
    if (!userType) {
      const { error: insertTypeError } = await supabaseAdmin
        .from("user_types")
        .insert({
          user_id: userId,
          user_type: "admin",
      ...
      if (insertTypeError) {
        return {
          success: false,
          error: `Error inserting user type: ${insertTypeError.message}`,
        };
      }
      ...
      if (updateTypeError) {
        return {
          success: false,
          error: `Error updating user type: ${updateTypeError.message}`,
        };
      }
    }
    ...
    if (adminUserError && adminUserError.code !== "PGRST116") {
      return {
        success: false,
        error: `Error checking admin user: ${adminUserError.message}`,
      };
    }
    ...
    if (!adminUser) {
      ...
      if (insertError) {
        return {
          success: false,
          error: `Error inserting admin user: ${insertError.message}`,
        };
      }
    }
    ...
    if (userRoleError && userRoleError.code !== "PGRST116") {
      return {
        success: false,
        error: `Error checking user role: ${userRoleError.message}`,
      };
    }
    ...
    if (!userRole) {
      ...
      if (insertRoleError) {
        return {
          success: false,
          error: `Error inserting user role: ${insertRoleError.message}`,
        };
      }
      ...
      if (updateRoleError) {
        return {
          success: false,
          error: `Error updating user role: ${updateRoleError.message}`,
        };
      }
    }
