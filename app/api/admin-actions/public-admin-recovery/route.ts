...
export async function recoverAdminUser(email: string, recoveryCode: string) {
  try {
    ...
    if (userError) {
      ...
      return {
        success: false,
        message: "Failed to list users: " + userError.message,
      };
    }
    ...
    if (insertError) {
      ...
      await supabase
        ...
        .catch((err) => {
          console.log(
            "Error inserting admin user via RPC, trying direct SQL",
            err,
          );
          return supabase.auth.admin.executeRaw(`
          INSERT INTO admin.users (id, email, name, updated_at)
    ...
    if (userTypeError) {
      ...
      await supabase
        ...
        .catch((err) => {
          console.log(
            "Error inserting user_types via RPC, trying direct SQL",
            err,
          );
          return supabase.auth.admin.executeRaw(`
          INSERT INTO user_types (user_id, user_type, email)
    ...
    if (userRoleError) {
      ...
      await supabase
        ...
        .catch((err) => {
          console.log(
            "Error inserting user_roles via RPC, trying direct SQL",
            err,
          );
          return supabase.auth.admin.executeRaw(`
          INSERT INTO user_roles (user_id, role, assigned_at, is_active)
    ...
    await supabase
      ...
      .catch((err) => {
        console.log(
          "Error creating recruit schema/tables, continuing anyway",
          err,
        );
    ...
    return {
      success: true,
      message:
        "Admin user recovery completed successfully. You should now be able to log in.",
      userId: userId,
    };
  } catch (error) {
