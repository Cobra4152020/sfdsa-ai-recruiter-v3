    ...
    try {
      ...
      if (error) {
        ...
        if (error.message && error.message.includes("does not exist")) {
          console.log(
            "Performance metrics table doesn't exist yet. Metrics will be logged to console only.",
          );
          return NextResponse.json({
            success: true,
            message: "Metric logged to console (table doesn't exist)",
          });
        }
        ...
        if (
          ...
          console.log(
            "RLS policy prevented metric insertion. This is expected for anonymous users.",
          );
          return NextResponse.json({
            success: true,
            message: "Metric logged to console (RLS policy)",
          });
        }
      ...
      return NextResponse.json({
        success: true,
        message: "Metric logged to console (database error)",
      });
    }
  } catch (error) {
