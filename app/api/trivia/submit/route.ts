...
export async function POST(req: Request) {
  try {
    const {
      ...
      gameMode = "normal",
      answers = [],
    } = (await req.json()) as TriviaSubmission;
...
async function checkAndAwardBadges(
  ...
  const hasBadge = (type: string) =>
    existingBadges?.some((b: { badge_type: string }) => b.badge_type === type);
  ...
  const fastCorrectAnswers = answers.filter(
    (a) => a.isCorrect && a.answerTime < 5,
  ).length;
  if (fastCorrectAnswers >= 10 && !hasBadge("speed-demon")) {
    await supabase
      .from("badges")
      .insert({ user_id: userId, badge_type: "speed-demon" })
      .single();
    badgesAwarded.push({
  ...
  if (count >= 5 && !hasBadge("trivia-enthusiast")) {
    await supabase
      .from("badges")
      .insert({ user_id: userId, badge_type: "trivia-enthusiast" })
      .single();
    badgesAwarded.push({
  ...
  if (perfectScores >= 3 && !hasBadge("trivia-master")) {
    await supabase
      .from("badges")
      .insert({ user_id: userId, badge_type: "trivia-master" })
      .single();
    badgesAwarded.push({
