...
export async function GET(request: Request) {
  try {
    ...
    const totalAmount = totalData.reduce(
      (sum: number, donation: { amount: string }) =>
        sum + Number.parseFloat(donation.amount),
      0,
    );
    ...
    return NextResponse.json({
      totalAmount,
      totalCount,
      periodAmount: periodData.reduce(
        (sum: number, donation: { amount: string }) =>
          sum + Number.parseFloat(donation.amount),
        0,
      ),
    ...
    return NextResponse.json(
      { error: "Error fetching donation analytics" },
      { status: 500 },
    );
  }
}
...
function formatChartData(
  donations: { created_at: string; amount: string }[],
  period: string,
) {
