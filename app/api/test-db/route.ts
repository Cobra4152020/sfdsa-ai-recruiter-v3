...
export async function GET() {
  try {
    ...
    const { data: connectionTest, error: connectionError } = await supabase.from('user_roles').select('count(*)', { count: 'exact', head: true })
    ...
    if (connectionError) {
      ...
      return NextResponse.json({ success: false, error: "Database connection failed", debug: debugInfo }, { status: 500 })
    }
    ...
    if (tableError) {
      ...
      return NextResponse.json({ success: false, error: "Failed to access user_roles table", debug: debugInfo }, { status: 500 })
    }

    debugInfo.tableStructure = tableInfo.length > 0 ? Object.keys(tableInfo[0]) : []
