    ...
    try {
      ...
      const formattedNFTAwards = (nftAwards || []).map((award) => {
        const awardDetails = NFT_AWARD_TIERS.find(
          (tier) => tier.id === award.nft_award_id,
        );
        return {
          ...award,
      ...
      const earnedThresholds = formattedNFTAwards.map(
        (award) => award.pointThreshold,
      );
        ...
        profile: {
          ...
          next_award: nextAward
            ? {
                ...nextAward,
                pointsNeeded:
                  nextAward.pointThreshold - (user.participation_count || 0),
              }
            : null,
        },
      });
...
export async function PATCH(
  ...
  try {
    ...
    if (bio && bio.length > 500) {
      return NextResponse.json(
        { success: false, message: "Bio must be less than 500 characters" },
        { status: 400 },
      );
    }
