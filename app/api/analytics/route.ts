...
export async function GET(request: Request) {
  ...
  try {
    switch (type) {
      case "growth":
        const { data: growthData, error: growthError } =
      ...
      case "geographic":
        const { data: geographicData, error: geographicError } =
          await getServiceSupabase().from("user_geographic_stats").select("*");
      ...
      case "monthly-report":
        const month = new Date(
          searchParams.get("month") || new Date().toISOString(),
        );
        const firstDay = startOfMonth(month);
        const lastDay = endOfMonth(month);
        const { data: reportData, error: reportError } =
          await getServiceSupabase().rpc("generate_monthly_report", {
