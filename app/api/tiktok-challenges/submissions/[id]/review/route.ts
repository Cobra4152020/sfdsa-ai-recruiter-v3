...
export async function GET(
  ...
  try {
    ...
    const submission =
      await TikTokChallengeService.getSubmissionById(submissionId);
...
export async function POST(
  ...
  try {
    ...
    // Review submission
    const submission = await TikTokChallengeService.reviewSubmission(
      id,
    ...
    return NextResponse.json(
      { error: "Failed to review TikTok challenge submission" },
      { status: 500 },
    );
  }
}
