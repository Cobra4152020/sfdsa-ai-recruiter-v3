...
export async function GET() {
  try {
    ...
    try {
      ...
      const hasCorrectRedirectUrl = redirectUrls.some((url) =>
        url.includes(new URL(siteUrl).hostname),
      );
    ...
    try {
      ...
      steps.push({
        ...
        success: tableExists,
        message: tableExists
          ? "Email confirmation tokens table exists"
          : "Email confirmation tokens table is missing",
    ...
    try {
      const testResponse = await fetch(
        `${constructUrl()}/api/volunteer-confirm?test=true`,
        {
          method: "GET",
        },
      );
    ...
    return NextResponse.json({
      ...
      message:
        error instanceof Error
          ? error.message
          : "Unknown error testing auth flow",
      steps: [
        {
          name: "Overall Test",
          success: false,
