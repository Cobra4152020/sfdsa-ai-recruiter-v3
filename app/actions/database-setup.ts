...
async function setupDatabase() {
  ...
  try {
    const dailyBriefingsSql = fs.readFileSync(
      path.join(process.cwd(), "migrations", "create_daily_briefings_tables.sql"),
      "utf8",
    )
    ...
    if (dailyBriefingsError) {
      console.error("Error setting up daily briefings tables:", dailyBriefingsError)
    ...
    errors.push(`Daily briefings tables: ${error instanceof Error ? error.message : String(error)}`)
  }
