    ...
    async function fetchData() {
      try {
        ...
        if (totalStats) {
          ...
          newStats.total_amount = totalStats.reduce(
            (sum: number, donation: DonationAmount) =>
        ...
        if (monthStats) {
          ...
          newStats.month_amount = monthStats.reduce(
            (sum: number, donation: DonationAmount) =>
              sum + (donation.amount || 0),
            0,
          );
        ...
        if (recurringStats) {
          ...
          newStats.recurring_amount = recurringStats.reduce(
            (sum: number, donation: DonationAmount) =>
              sum + (donation.amount || 0),
            0,
          );
    ...
    <div className="container py-10">
      ...
      <div className="grid gap-6 md:grid-cols-3 mb-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ${stats?.month_amount || 0}
            </div>
            <p className="text-xs text-muted-foreground">
              {stats?.month_count || 0} donations
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ${stats?.recurring_amount || 0}/mo
            </div>
            <p className="text-xs text-muted-foreground">
              {stats?.recurring_count || 0} subscriptions
            </p>
          </CardContent>
        </Card>
      </div>
