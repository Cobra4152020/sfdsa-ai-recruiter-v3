...
export default function EmailDiagnosticsPage() {
  ...
  const testVolunteerConfirmation = async () => {
    ...
    try {
      const res = await fetch("/api/admin/email-diagnostics/test-volunteer-confirmation", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email }),
      })
      ...
      setTestResults({
        ...testResults,
        volunteer: { success: false, error: "Failed to test volunteer confirmation" },
      })
    } finally {
  ...
  return (
    <div className="container mx-auto py-10">
      ...
      <p className="text-gray-600 mb-8">
        Use this tool to diagnose issues with email sending, particularly for volunteer recruiter confirmation emails.
      </p>
        ...
        <TabsContent value="configuration">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Verify that all required environment variables and configurations are set correctly.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {testResults.config ? (
                <div className="space-y-4">
                  <Alert variant={testResults.config.success ? "default" : "destructive"}>
                    <div className="flex items-center gap-2">
                      ...
                      <AlertTitle>
                        {testResults.config.success ? "Configuration Valid" : "Configuration Error"}
                      </AlertTitle>
                    </div>
                    <AlertDescription className="mt-2">{testResults.config.message}</AlertDescription>
                  </Alert>
                ...
                <p className="text-gray-500 italic">Run the test to see configuration status.</p>
              )}
            </CardContent>
            <CardFooter>
        ...
        <TabsContent value="delivery">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Send a test email to verify that the email delivery system is working correctly.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {testResults.delivery ? (
                <div className="space-y-4">
                  <Alert variant={testResults.delivery.success ? "default" : "destructive"}>
                    <div className="flex items-center gap-2">
                      ...
                      <AlertTitle>
                        {testResults.delivery.success ? "Email Sent Successfully" : "Email Delivery Failed"}
                      </AlertTitle>
                    </div>
                    <AlertDescription className="mt-2">{testResults.delivery.message}</AlertDescription>
                  </Alert>
                ...
                <p className="text-gray-500 italic">Enter an email address and run the test to check delivery.</p>
              )}
            </CardContent>
            <CardFooter>
              <Button onClick={testEmailDelivery} disabled={isLoading.delivery || !email}>
                {isLoading.delivery ? (
                  <>
                    <RefreshCw className="mr-2 h-4 w-4 animate-spin" />
                    Sending...
        ...
        <TabsContent value="volunteer">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Test the volunteer recruiter confirmation email flow specifically.</CardDescription>
            </CardHeader>
            <CardContent>
              {testResults.volunteer ? (
                <div className="space-y-4">
                  <Alert variant={testResults.volunteer.success ? "default" : "destructive"}>
                    <div className="flex items-center gap-2">
                    ...
                    <AlertDescription className="mt-2">{testResults.volunteer.message}</AlertDescription>
                  </Alert>
                ...
                <p className="text-gray-500 italic">
                  Enter an email address and run the test to check volunteer confirmation.
                </p>
              )}
            </CardContent>
            <CardFooter>
              <Button onClick={testVolunteerConfirmation} disabled={isLoading.volunteer || !email}>
                {isLoading.volunteer ? (
                  <>
                    <RefreshCw className="mr-2 h-4 w-4 animate-spin" />
                    Testing...
        ...
        <TabsContent value="auth">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Test the complete authentication flow including registration, confirmation, and access control.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {testResults.auth ? (
                <div className="space-y-4">
                  <Alert variant={testResults.auth.success ? "default" : "destructive"}>
                    <div className="flex items-center gap-2">
                      ...
                      <AlertTitle>
                        {testResults.auth.success ? "Auth Flow Valid" : "Auth Flow Issues Detected"}
                      </AlertTitle>
                    </div>
                    <AlertDescription className="mt-2">{testResults.auth.message}</AlertDescription>
                  </Alert>
                  ...
                  {testResults.auth.steps && (
                    <div className="mt-4">
                      ...
                      <div className="space-y-2">
                        {testResults.auth.steps.map((step: any, index: number) => (
                          <div
                            key={index}
                ...
                <p className="text-gray-500 italic">Run the test to verify the complete auth flow.</p>
              )}
            </CardContent>
            <CardFooter>
