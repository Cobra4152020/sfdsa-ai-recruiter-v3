  ...
  jsFiles.forEach((file) => {
    ...
    nonIdParams.forEach((param) => {
      ...
      if (content.includes(`params.${param}`)) {
        content = content.replace(new RegExp(`params\\.${param}`, "g"), "params.id")
        fileModified = true
      }

      // Fix { param } destructuring
      if (content.includes(`{ ${param} }`)) {
        content = content.replace(new RegExp(`\\{ ${param} \\}`, "g"), "{ id }")
        fileModified = true
      }
      ...
      if (content.includes(`${param}: string`)) {
        content = content.replace(new RegExp(`${param}: string`, "g"), "id: string")
        fileModified = true
      }
    })
