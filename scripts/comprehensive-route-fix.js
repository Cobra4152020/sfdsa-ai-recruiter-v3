...
function updateFileContent(filePath, oldParam, newParam = "id") {
  ...
  content = content.replace(new RegExp(`params: {[^}]*${oldParam}[^}]*}`, "g"), (match) =>
    match.replace(oldParam, newParam),
  )

  // Replace parameter in destructuring
  content = content.replace(new RegExp(`{ ${oldParam} }`, "g"), `{ ${newParam} }`)

  // Replace parameter in direct usage
  content = content.replace(new RegExp(`params\\.${oldParam}`, "g"), `params.${newParam}`)
