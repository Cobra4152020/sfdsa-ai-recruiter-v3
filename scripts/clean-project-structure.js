...
function createCleanDynamicRouteDirs() {
  ...
  const apiBadgesDir = path.join(__dirname, "..", "app", "api", "badges", "[id]")
  fs.mkdirSync(apiBadgesDir, { recursive: true })

  // Create app/api/user-badge/[id]
  const apiUserBadgeDir = path.join(__dirname, "..", "app", "api", "user-badge", "[id]")
  fs.mkdirSync(apiUserBadgeDir, { recursive: true })

  // Create app/api/nft-awards/[id]
  const apiNftAwardsDir = path.join(__dirname, "..", "app", "api", "nft-awards", "[id]")
