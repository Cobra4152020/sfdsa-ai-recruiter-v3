export async function databaseSchemaVerification(params: unknown) {
  try {
    const response = await fetch(
      `/api/admin-actions/database-schema-verification`,
      {
        method: "POST",
        headers: {
...
export async function fixSpecificIssues(issueIndices: number[]) {
  return databaseSchemaVerification({
    action: "fixSpecificIssues",
    issueIndices,
  });
}
