...
export class LoginAuditService {
  ...
  private initializeFindings() {
    ...
    this.addFinding({
      ...
      description: "The admin login page allows anyone to access the admin dashboard without proper authentication.",
      affectedRoles: ["admin"],
      recommendation: "Implement proper authentication for admin access with role-based validation.",
      fileReferences: ["app/admin-login/page.tsx", "components/admin-login-form.tsx"],
    })
    ...
    this.addFinding({
      ...
      description: "Social login flows don't properly validate user identity before granting access.",
      affectedRoles: ["recruit", "volunteer"],
      recommendation: "Enhance social login validation with proper identity verification and role mapping.",
    ...
    this.addFinding({
      ...
      description: "Different user roles have inconsistent login UI and workflows, causing user confusion.",
      affectedRoles: ["recruit", "volunteer", "admin"],
      recommendation: "Standardize login UI and flows while maintaining role-specific functionality.",
      fileReferences: ["components/auth-form.tsx", "components/login-form.tsx", "components/admin-login-form.tsx"],
    })
    ...
    this.addFinding({
      ...
      description: "Users are frequently logged out, requiring repeated authentication.",
      affectedRoles: ["recruit", "volunteer"],
      recommendation: "Implement secure persistent sessions with appropriate timeout policies.",
    ...
    this.addFinding({
      ...
      description: "Login process takes longer than optimal, affecting user experience.",
      affectedRoles: ["recruit", "volunteer", "admin"],
      recommendation: "Optimize authentication flow and reduce database queries during login.",
    ...
    this.addFinding({
      ...
      description: "The 50-point bonus for new users is not consistently applied across all registration methods.",
      affectedRoles: ["recruit"],
      recommendation: "Standardize point allocation across all registration pathways.",
      fileReferences: ["app/api/auth/register/route.ts", "lib/unified-auth-service.ts"],
    })
    ...
    this.addFinding({
      ...
      affectedRoles: ["recruit", "volunteer"],
      recommendation: "Expand social login options to include additional popular providers.",
