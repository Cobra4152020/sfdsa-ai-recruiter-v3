    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (!errorText.includes("violates row-level security policy")) {
          console.warn(
            `Performance metric reporting failed (${response.status}): ${errorText}`,
          );
        }
...
// Add this interface at the top or near the memory usage function
...
export function trackMemoryUsage(): void {
  ...
  try {
    ...
    if (memory) {
      ...
      // const totalHeapSize = memory.totalJSHeapSize; // Commented out unused variable
