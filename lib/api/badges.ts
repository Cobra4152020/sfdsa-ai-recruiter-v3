...
export async function getBadge(badgeId: string): Promise<BadgeWithProgress> {
  try {
    ...
    if (badgeError) throw new BadgeError("Failed to fetch badge", badgeError.code)
    ...
    if (progressError && progressError.code !== "PGRST116") { // Ignore not found error
      throw new BadgeError("Failed to fetch badge progress", progressError.code)
    }
...
export async function getAllBadges(): Promise<BadgeWithProgress[]> {
  try {
    ...
    if (badgesError) throw new BadgeError("Failed to fetch badges", badgesError.code)
    if (!badges) return []
    ...
    if (progressError) throw new BadgeError("Failed to fetch badge progress", progressError.code)
