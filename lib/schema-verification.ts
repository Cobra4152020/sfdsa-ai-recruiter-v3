...
export async function verifyDatabaseSchema(): Promise<SchemaVerificationResult> {
  try {
    ...
    const { data: tableExistsFunction, error: tableExistsFunctionError } =
      await supabase.rpc("function_exists", {
    ...
    const { data: columnExistsFunction, error: columnExistsFunctionError } =
      await supabase.rpc("function_exists", {
    ...
    const {
      data: constraintExistsFunction,
      error: constraintExistsFunctionError,
    } = await supabase.rpc("function_exists", {
    ...
    for (const expectedTable of expectedSchema.tables) {
      ...
      if (tableExistsError) {
        console.error(
          `Error checking if table ${expectedTable.name} exists:`,
          tableExistsError,
        );
        ...
        if (!tableExists) {
          ...
          for (const expectedColumn of expectedTable.columns) {
            const { data: columnExists, error: columnExistsError } =
            ...
            if (columnExistsError) {
              console.error(
                `Error checking if column ${expectedColumn.name} exists:`,
                columnExistsError,
              );
              tableInfo.issues.push({
                type: "other",
          ...
          if (expectedTable.constraints) {
            for (const expectedConstraint of expectedTable.constraints) {
              const { data: constraintExists, error: constraintExistsError } =
                await supabase.rpc("constraint_exists", {
                  table_name: expectedTable.name,
              ...
              if (constraintExistsError) {
                console.error(
                  `Error checking if constraint ${expectedConstraint.name} exists:`,
                  constraintExistsError,
                );
                tableInfo.issues.push({
                  type: "other",
    ...
    if (userRolesTable && userRolesTable.exists) {
      ...
      if (!constraintsError && constraints) {
        for (const constraint of constraints) {
          // Get constraint definition
          const { data: constraintDef, error: constraintDefError } =
 ...
 */
export async function checkTableIssues(
  tableName: string,
