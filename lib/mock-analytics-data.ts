import type { UserGrowthData, UserEngagementData, ConversionData, GeographicData, RetentionData, BadgeDistributionData, UserActivitySummary } from '../app/types/analytics';
...
export const mockAnalyticsData = {
  ...
  conversion: Array.from({ length: 10 }, (_, i) => ({
    ...
    conversion_rate: Math.round((Math.pow(0.8, i) * 100 + Math.random() * 5) * 10) / 10
  })) as ConversionData[],
  geographic: Array.from({ length: 10 }, (_, i) => ({
    zip_code: `941${String(i).padStart(2, '0')}`,
    region: ['San Francisco', 'Oakland', 'San Jose', 'Berkeley', 'Palo Alto', 'Mountain View', 'Fremont', 'Richmond', 'Daly City', 'San Mateo'][i],
  ...
  badges: Array.from({ length: 8 }, (_, i) => ({
    badge_id: `badge-${i + 1}`,
    badge_name: ['Early Bird', 'Quick Learner', 'Team Player', 'Problem Solver', 'Community Leader', 'Top Performer', 'Innovator', 'Expert'][i],
    count: Math.floor(Math.random() * 500) + 100,
    percentage: Math.round((Math.random() * 20 + 5) * 10) / 10
