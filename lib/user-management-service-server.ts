...
import type { UserRole, UserStatus, UserBasic, RecruitUser, VolunteerUser, AdminUser, UserWithRole, UserStats } from './user-management-service'
...
export async function getAllUsers(options: {
  ...
  try {
    ...
    if (volunteersData.data) {
      (volunteersData.data as VolunteerUser[]).forEach((volunteer: VolunteerUser) => {
        users.push({
          ...volunteer,
          user_type: "volunteer",
...
export async function getUserById(id: string): Promise<UserWithRole | null> {
  try {
    ...
    switch (userType.user_type) {
      case "recruit":
        ;({ data: userData, error } = await supabaseAdmin.from("recruit.users").select("*").eq("id", id).single())
        break
      case "volunteer":
        ;({ data: userData, error } = await supabaseAdmin.from("volunteer.recruiters").select("*").eq("id", id).single())
        break
      case "admin":
        ;({ data: userData, error } = await supabaseAdmin.from("admin.users").select("*").eq("id", id).single())
        break
    }
    if (error) throw error
    if (!userData) return null
    let status: UserStatus = "pending"
    if (userType.user_type === "volunteer" && (userData as VolunteerUser).is_active !== undefined) {
...
export async function updateUser(
  ...
  try {
    ...
    switch (userType.user_type) {
      case "recruit":
        ;({ error } = await supabaseAdmin.from("recruit.users").update(data).eq("id", id))
        break
      case "volunteer":
        ;({ error } = await supabaseAdmin.from("volunteer.recruiters").update(data).eq("id", id))
        break
      case "admin":
        ;({ error } = await supabaseAdmin.from("admin.users").update(data).eq("id", id))
        break
    }
    if (error) throw error
    return { success: true }
...
export async function deleteUser(id: string): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    switch (userType.user_type) {
      case "recruit":
        ;({ error } = await supabaseAdmin.from("recruit.users").delete().eq("id", id))
        break
      case "volunteer":
        ;({ error } = await supabaseAdmin.from("volunteer.recruiters").delete().eq("id", id))
        break
      case "admin":
        ;({ error } = await supabaseAdmin.from("admin.users").delete().eq("id", id))
        break
    }
    if (error) throw error
    ;({ error } = await supabaseAdmin.from("user_types").delete().eq("user_id", id))
    if (error) throw error
    return { success: true }
  } catch (error: unknown) {
...
export async function changeUserRole(id: string, newRole: UserRole): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    switch (newRole) {
      case "recruit":
        ;({ error } = await supabaseAdmin.from("recruit.users").insert(userData))
        break
      case "volunteer":
        ;({ error } = await supabaseAdmin.from("volunteer.recruiters").insert(userData))
        break
      case "admin":
    ...
    if (error) throw error
    ;({ error } = await supabaseAdmin.from("user_types").update({ user_type: newRole }).eq("user_id", id))
    if (error) throw error
    return { success: true }
  } catch (error: unknown) {
...
export async function rejectVolunteerRecruiter(id: string): Promise<{ success: boolean; error?: string }> {
  try {
    const supabaseAdmin = getServiceSupabase()
    const { error } = await supabaseAdmin.from("volunteer.recruiters").delete().eq("id", id)
    if (error) throw error
    return { success: true }
  } catch (error: unknown) {
