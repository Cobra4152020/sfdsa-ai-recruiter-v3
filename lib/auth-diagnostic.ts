...
export async function diagnoseUserAuth(email: string): Promise<UserDiagnosticResult> {
  ...
  try {
    ...
    if (result.authUserExists && !result.userProfileExists) {
      result.discrepancies.push("User exists in auth.users but not in user_profiles")
    }

    if (!result.authUserExists && result.userProfileExists) {
      result.discrepancies.push("User exists in user_profiles but not in auth.users")
    }

    if (result.authUserExists && result.userProfileExists && result.authUserId !== result.userProfileId) {
      result.discrepancies.push("User ID mismatch between auth.users and user_profiles")
    }

    if (result.userProfileExists && result.isVolunteerRecruiter && !result.hasUserRole) {
      result.discrepancies.push("User marked as volunteer recruiter but has no role assignment")
    }

    if (result.userProfileExists && !result.isVolunteerRecruiter && result.hasUserRole) {
      result.discrepancies.push("User has volunteer recruiter role but not marked as such in profile")
    }

    if (authUser && userProfile && !!authUser.email_confirmed_at !== !!userProfile.is_email_confirmed) {
      result.discrepancies.push("Email confirmation status mismatch between auth.users and user_profiles")
    }

    return result
  } catch (error) {
    console.error("Error diagnosing user auth:", error)
    result.discrepancies.push(`Error during diagnosis: ${error instanceof Error ? error.message : String(error)}`)
    return result
  }
}
