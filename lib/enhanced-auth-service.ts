 ...
 *
 * This service provides a unified, secure authentication system for all user roles
 * while addressing the issues identified in the audit.
 */
  ...
  async signInWithMagicLink(email: string, redirectUrl?: string): Promise<AuthResult> {
    try {
      ...
      if (!finalRedirectUrl && userData) {
        finalRedirectUrl = this.getRedirectUrlForUserRole(userData.user_type as UserRole)
      }
      ...
      if (error) {
        await this.recordLoginError({
          email,
          errorType: "auth/magic-link-error",
          errorMessage: error.message,
          method: "magic_link",
        })
