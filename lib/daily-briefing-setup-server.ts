...
import type { DailyBriefing } from "./daily-briefing-setup"
...
export async function setupDailyBriefingsTable() {
  ...
  try {
    ...
    if (checkError) {
      console.error("Error checking if daily_briefings table exists:", checkError)
      ...
      if (listError) {
        ...
        return { success: false, message: "Failed to check if daily_briefings table exists" }
      }
    ...
    if (createError) {
      ...
      return { success: false, message: "Failed to create daily_briefings table" }
    }

    return { success: true, message: "daily_briefings table created successfully" }
  } catch (error) {
...
export async function addSampleBriefing() {
  ...
  try {
    ...
    if (checkError) {
      ...
      return { success: false, message: "Failed to check for existing briefing" }
    }
    ...
    return { success: true, message: "Sample briefing added successfully", briefing: data }
  } catch (error) {
