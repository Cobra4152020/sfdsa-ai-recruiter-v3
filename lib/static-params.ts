...
export async function generateTikTokChallengeStaticParams() {
  ...
  return (
    (challenges as TikTokChallenge[] | null)?.map((challenge) => ({
...
export async function generateTikTokSubmissionStaticParams() {
  ...
  return (
    (submissions as TikTokSubmission[] | null)?.map((submission) => ({
...
export async function generateDonationStaticParams() {
  ...
  return (
    (donations as Donation[] | null)?.map((donation) => ({
      id: donation.id,
    })) || []
  );
}
