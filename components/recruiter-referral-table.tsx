...
export function RecruiterReferralTable({ referrals }: RecruiterReferralTableProps) {
      ...
      <CardHeader>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <CardDescription>Detailed information about your referrals</CardDescription>
          </div>
                    ...
                    <TableRow key={referral.id}>
                      ...
                      <TableCell>
                        <div className="flex flex-col">
                          ...
                          {referral.daysToHire && (
                            <span className="text-xs text-green-600">{referral.daysToHire} days to hire</span>
                          )}
                          {referral.daysInProcess && (
                            <span className="text-xs text-gray-500">{referral.daysInProcess} days in process</span>
                          )}
                        </div>
                      </TableCell>
                      <TableCell className="text-right">
                        <div className="flex justify-end space-x-2">
                          <Button variant="ghost" size="sm" onClick={() => toggleReferralDetails(referral.id)}>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="flex items-start">
                                  <div className="w-32 text-sm font-medium text-gray-500">Source:</div>
                                  <div className="text-sm">{referral.source}</div>
                                </div>
                                <div className="flex items-start">
                                  <div className="w-32 text-sm font-medium text-gray-500">Status:</div>
                                  <div className="text-sm">{referral.status}</div>
                                </div>
                                <div className="flex items-start">
                                  <div className="w-32 text-sm font-medium text-gray-500">Referred On:</div>
                                  <div className="text-sm">{new Date(referral.date).toLocaleDateString()}</div>
                                </div>
                                {referral.daysToHire && (
                                  <div className="flex items-start">
                                    <div className="w-32 text-sm font-medium text-gray-500">Days to Hire:</div>
                                    <div className="text-sm">{referral.daysToHire} days</div>
                                  </div>
                                )}
                                {referral.daysInProcess && (
                                  <div className="flex items-start">
                                    <div className="w-32 text-sm font-medium text-gray-500">Days in Process:</div>
                                    <div className="text-sm">{referral.daysInProcess} days</div>
                                  </div>
                                )}
                              </div>
                            </div>
                            ...
                            <div>
                              ...
                              <div className="space-y-4">
                                <div className="flex">
                                  ...
                                  <div>
                                    <div className="font-medium">Referral Created</div>
                                    <div className="text-sm text-gray-500">
                                      {new Date(referral.date).toLocaleDateString()}
                                    </div>
                                    <div className="text-sm text-gray-600 mt-1">
                                      Initial referral created via {referral.source}
                                    </div>
                                  </div>
                                </div>
                                ...
                                referral.status === "Rejected" ? (
                                  <div className="flex">
                                    ...
                                    <div>
                                      ...
                                      <div className="text-sm text-gray-500">
                                        {new Date(
                                          new Date(referral.date).getTime() + 2 * 24 * 60 * 60 * 1000,
                                        ).toLocaleDateString()}
                                      </div>
                                      <div className="text-sm text-gray-600 mt-1">
                                        Candidate was contacted by recruitment team
                                      </div>
                                    </div>
                                  </div>
                                ) : null}
                                ...
                                referral.status === "Rejected" ? (
                                  <div className="flex">
                                    ...
                                    <div>
                                      ...
                                      <div className="text-sm text-gray-500">
                                        {new Date(
                                          new Date(referral.date).getTime() + 5 * 24 * 60 * 60 * 1000,
                                        ).toLocaleDateString()}
                                      </div>
                                      <div className="text-sm text-gray-600 mt-1">
                                ...
                                referral.status === "Rejected" ? (
                                  <div className="flex">
                                    ...
                                    <div>
                                      ...
                                      <div className="text-sm text-gray-500">
                                        {new Date(
                                          new Date(referral.date).getTime() + 10 * 24 * 60 * 60 * 1000,
                                        ).toLocaleDateString()}
                                      </div>
                                      <div className="text-sm text-gray-600 mt-1">
                                      ...
                                      <div className="text-sm text-gray-500">
                                        {new Date(
                                          new Date(referral.date).getTime() +
                                            (referral.daysToHire || 20) * 24 * 60 * 60 * 1000,
                                        ).toLocaleDateString()}
                                      </div>
                                      <div className="text-sm text-gray-600 mt-1">Candidate was successfully hired</div>
                                    </div>
                                  </div>
                                ) : null}
                                ...
                                {referral.status === "Rejected" ? (
                                  <div className="flex">
                                    ...
                                    <div>
                                      ...
                                      <div className="text-sm text-gray-500">
                                        {new Date(
                                          new Date(referral.date).getTime() + 15 * 24 * 60 * 60 * 1000,
                                        ).toLocaleDateString()}
                                      </div>
                                      <div className="text-sm text-gray-600 mt-1">
