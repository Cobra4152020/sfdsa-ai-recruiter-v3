...
export function ChallengeLeaderboard() {
  ...
  const fetchLeaderboard = async () => {
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to load TikTok challenge leaderboard. Please try again.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
    ...
    return (
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-10">
          ...
          <h3 className="text-lg font-medium text-gray-900">No challenge entries yet</h3>
          <p className="text-gray-500 mt-1">
            Be the first to complete a TikTok challenge and earn your place on the leaderboard!
          </p>
        </CardContent>
      </Card>
    )
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {leaderboard.slice(0, 3).map((entry, index) => (
          <Card key={entry.userId} className={`overflow-hidden ${index === 0 ? "border-yellow-400" : ""}`}>
            ...
            <CardContent className="pt-6">
              <div className="flex flex-col items-center text-center">
                <div className="relative mb-4">
                  ...
                  <div className={`absolute -top-3 -right-3 p-1.5 rounded-full ${getTrophyBgColor(index)}`}>
                    {getTrophyIcon(index)}
                  </div>
                </div>
                ...
                <div className="mt-4 text-sm text-gray-600">
                  <p>Latest challenge: "{entry.challengeDetails[0]?.challengeTitle || "N/A"}"</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            {leaderboard.slice(3).map((entry, index) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 truncate">{entry.challengesCompleted} challenges completed</p>
                </div>
