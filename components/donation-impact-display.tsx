...
export function DonationImpactDisplay({ amount, customAmount }: DonationImpactDisplayProps) {
  const [impact, setImpact] = useState<string>("")
  const [secondaryImpact, setSecondaryImpact] = useState<string>("")

  useEffect(() => {
    const donationAmount = customAmount ? Number.parseFloat(customAmount) : Number.parseFloat(amount as string)
    ...
    if (donationAmount < 10) {
      ...
      setImpact("Provides comprehensive information packages for 5 potential recruits")
    } else if (donationAmount < 50) {
      setImpact("Helps enhance Sgt. Ken's AI capabilities with updated information")
    ...
    if (donationAmount >= 25 && donationAmount < 50) {
      setSecondaryImpact("Also provides information packages for 10+ potential recruits")
      ...
      setSecondaryImpact("Also supports multiple community outreach initiatives")
    } else if (donationAmount >= 250) {
      setSecondaryImpact("Also funds multiple recruitment workshops and outreach events")
    } else {
      setSecondaryImpact("")
    }
  }, [amount, customAmount])
  ...
  return (
    <div className="mt-4 bg-[#0A3C1F]/5 p-4 rounded-lg">
      <h3 className="font-semibold text-[#0A3C1F] mb-2">Your Donation Impact:</h3>
      <div className="flex items-start mb-1">
        <Check className="h-5 w-5 text-[#0A3C1F] mr-2 mt-0.5 flex-shrink-0" />
