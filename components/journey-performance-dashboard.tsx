...
interface DurationData extends ChartDataPoint {
  name: string;
  avgDuration: number;
  p95Duration: number;
}

interface FunnelData extends ChartDataPoint {
  step: string;
  users: number;
  completionRate: number;
}
...
export function JourneyPerformanceDashboard({
  ...
  const [journeyFunnels, setJourneyFunnels] = useState<JourneyFunnel[]>(
    initialData?.funnels || [],
  );
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `/api/performance/journeys?timeRange=${timeRange}&journey=${selectedJourney}`,
        );
  ...
  const journeyNames = Array.from(
    new Set(journeySummaries.map((summary) => summary.journey_name)),
  );
  ...
  const filteredSteps =
    selectedJourney === "all"
      ? journeySteps
      : journeySteps.filter((step) => step.journey_name === selectedJourney);

  const filteredFunnels =
    selectedJourney === "all"
      ? journeyFunnels
      : journeyFunnels.filter(
          (funnel) => funnel.journey_name === selectedJourney,
        );

  // Prepare data for charts
  const completionRateData: ChartData[] = journeySummaries.map((summary) => ({
  ...
  const durationData: DurationData[] = journeySummaries.map((summary) => ({
    name: formatJourneyName(summary.journey_name),
  ...
  const funnelData: FunnelData[] = filteredFunnels
        ...
        <TabsContent value="overview" className="space-y-6 mt-6">
            ...
            {loading ? (
              ...
              <>
                <Card>
                  ...
                  <CardContent>
                    <div className="text-3xl font-bold">
                      {journeySummaries
                        .reduce(
                          (sum, journey) => sum + journey.total_journeys,
                          0,
                        )
                        .toLocaleString()}
                    </div>
                    <p className="text-sm text-muted-foreground mt-2">
                      {journeySummaries
                        .reduce((sum, journey) => sum + journey.unique_users, 0)
                        .toLocaleString()}{" "}
                      unique users
                    </p>
                  </CardContent>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-3xl font-bold">
                      {calculateOverallCompletionRate(journeySummaries)}%
                    </div>
                    <p className="text-sm text-muted-foreground mt-2">
                      {journeySummaries
                        .reduce(
                          (sum, journey) => sum + journey.completed_journeys,
                          0,
                        )
                        .toLocaleString()}{" "}
                      completed journeys
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-3xl font-bold">
                      {formatDuration(
                        calculateWeightedAverageDuration(journeySummaries),
                      )}
                    </div>
                    <p className="text-sm text-muted-foreground mt-2">
                      P95:{" "}
                      {formatDuration(
                        calculateWeightedP95Duration(journeySummaries),
                      )}
                    </p>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-3xl font-bold">
                      {calculateWeightedAverageSteps(journeySummaries).toFixed(
                        1,
                      )}
                    </div>
                    <p className="text-sm text-muted-foreground mt-2">
                      Across {journeySummaries.length} journey types
                    </p>
                  </CardContent>
                </Card>
              </>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Percentage of users completing each journey
                </CardDescription>
              </CardHeader>
              <CardContent>
                {loading ? (
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Average time to complete each journey
                </CardDescription>
              </CardHeader>
              <CardContent>
                {loading ? (
                  ...
                  <div className="h-[300px]">
                    <BarChart
                      data={durationData}
                      index="name"
                      categories={["avgDuration", "p95Duration"]}
                      colors={["#0A3C1F", "#4CAF50"]}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Detailed metrics for each journey type
              </CardDescription>
            </CardHeader>
            <CardContent>
              {loading ? (
                      ...
                      {journeySummaries.map((summary) => (
                        <TableRow key={summary.journey_name}>
                          <TableCell className="font-medium">
                            {formatJourneyName(summary.journey_name)}
                          </TableCell>
                          <TableCell className="text-right">
                            {summary.total_journeys.toLocaleString()}
                          </TableCell>
                          <TableCell className="text-right">
                            <JourneyCompletionBadge
                              rate={
                                (summary.completed_journeys /
                          ...
                          <TableCell className="text-right">
                            {formatDuration(summary.p95_duration)}
                          </TableCell>
                          <TableCell className="text-right">
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {filteredFunnels
                          ...
                          .map((funnel) => (
                            <TableRow
                              key={`${funnel.journey_name}-${funnel.step_number}`}
                            >
                              <TableCell className="font-medium">
                                {funnel.step_number}. {funnel.step_name}
                              </TableCell>
                              <TableCell className="text-right">
                                {funnel.step_count.toLocaleString()}
                              </TableCell>
                              <TableCell className="text-right">
                                <JourneyCompletionBadge
                              ...
                              <TableCell className="text-right">
                                {funnel.step_number === 1
                                  ? "-"
                                  : `${(100 - funnel.step_retention_rate).toFixed(1)}%`}
                              </TableCell>
                              <TableCell className="text-right">
                                {funnel.step_number === 1
                                  ? "100%"
                                  : `${funnel.step_retention_rate.toFixed(1)}%`}
                              </TableCell>
                            </TableRow>
                          ))}
                      </TableBody>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filteredSteps
                        ...
                        .map((step) => (
                          <TableRow
                            key={`${step.journey_name}-${step.step_name}-${step.step_number}`}
                          >
                            <TableCell className="font-medium">
                              {selectedJourney === "all"
                            ...
                            <TableCell className="text-right">
                              {formatDuration(step.median_duration)}
                            </TableCell>
                            <TableCell className="text-right">
                              {formatDuration(step.p95_duration)}
                            </TableCell>
                            <TableCell>
                              <StepPerformanceBadge
                                duration={step.avg_duration}
                                journeyType={step.journey_name}
                              />
                            </TableCell>
                          </TableRow>
                        ))}
...
function calculateOverallCompletionRate(summaries: JourneySummary[]): string {
  const totalJourneys = summaries.reduce(
    (sum, journey) => sum + journey.total_journeys,
    0,
  );
  const completedJourneys = summaries.reduce(
    (sum, journey) => sum + journey.completed_journeys,
    0,
  );
...
function calculateWeightedAverageDuration(summaries: JourneySummary[]): number {
  const totalCompletedJourneys = summaries.reduce(
    (sum, journey) => sum + journey.completed_journeys,
    0,
  );
  ...
  const weightedSum = summaries.reduce(
    (sum, journey) => sum + journey.avg_duration * journey.completed_journeys,
    0,
  );
...
function calculateWeightedP95Duration(summaries: JourneySummary[]): number {
  ...
  const weightedSum = summaries.reduce(
    (sum, journey) => sum + journey.p95_duration * journey.completed_journeys,
    0,
  );
...
function calculateWeightedAverageSteps(summaries: JourneySummary[]): number {
  ...
  const weightedSum = summaries.reduce(
    (sum, journey) => sum + journey.avg_steps * journey.total_journeys,
    0,
  );
