...
// Type for NetworkInformation (not always available in TS DOM lib)
  ...
  useEffect(() => {
    ...
    const onFirstInput = (event: PerformanceEventTiming) => {
      reportPerformanceMetric({
        ...
        rating:
          event.processingStart - event.startTime < 100
            ? "good"
            : "needs-improvement",
      });
    };
    ...
    const onLargestContentfulPaint = (event: PerformanceEntry) => {
      reportPerformanceMetric({
        name: "LCP",
    ...
    let clsValue = 0;
    const onLayoutShift = (
      event: PerformanceEntry & { value: number; hadRecentInput: boolean },
    ) => {
    ...
    const lcpObserver = new PerformanceObserver((entryList) => {
      for (const entry of entryList.getEntries()) {
        if (entry.entryType === "largest-contentful-paint") {
          onLargestContentfulPaint(entry as PerformanceEntry);
        }
      }
    });
