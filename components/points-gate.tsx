...
export function PointsGate({
  ...
  if (isAuthenticated === null) {
    // Loading state
    return (
      <div className="animate-pulse bg-gray-200 dark:bg-gray-700 h-32 rounded-md"></div>
    );
  }
  ...
  if (userPoints < requiredPoints) {
    return (
      <div className="border border-gray-200 dark:border-gray-700 rounded-md p-6 text-center">
        <p className="mb-2">
          You need {requiredPoints} points to access this content.
        </p>
        <p className="text-sm text-gray-500 dark:text-gray-400">
          You currently have {userPoints} points. Earn more by participating in
          activities.
        </p>
      </div>
    );
