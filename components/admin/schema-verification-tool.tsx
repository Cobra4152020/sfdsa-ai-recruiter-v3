...
export function SchemaVerificationTool() {
  const [verificationResult, setVerificationResult] =
    useState<SchemaVerificationResult | null>(null);
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({
          title: "Fix Failed",
          description:
            result.error || `Failed to fix issues for table ${tableName}`,
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const countTotalIssues = () => {
    ...
    const tableIssuesCount = verificationResult.tables.reduce(
                          ...
                          {table.exists && table.issues.length > 0 && (
                            <CardFooter className="py-2">
                              <Button
                                size="sm"
                                onClick={() =>
                                  handleFixTableIssues(table.table_name)
                                }
                                disabled={isFixing}
                              >
                ...
                <CardFooter>
                  {countTotalIssues() > 0 && (
                    ...
                    >
                      ...
                      {isFixing
                        ? "Fixing Issues..."
                        : `Fix All Issues (${countTotalIssues()})`}
                    </Button>
                  )}
                </CardFooter>
              </Card>
        ...
        <TabsContent value="fixes" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Select specific issues to fix from the list below
              </CardDescription>
            </CardHeader>
            <CardContent>
              {verificationResult ? (
                    ...
                    <div className="space-y-4">
                      ...
                      {verificationResult.globalIssues.length > 0 && (
                        <div>
                          ...
                          <ul className="space-y-2">
                            {verificationResult.globalIssues.map(
                              (issue, index) => (
                                <li
                                  key={index}
                        ...
                        (table, tableIndex) =>
                          table.issues.length > 0 && (
                            <div key={tableIndex}>
                              ...
                              <ul className="space-y-2">
                                {table.issues.map((issue, issueIndex) => {
                                  const globalIssuesCount =
                                    verificationResult.globalIssues.length;
                                  const previousTablesIssuesCount =
                                    verificationResult.tables
                                      .slice(0, tableIndex)
                                      .reduce(
                                        (count, t) => count + t.issues.length,
                                        0,
                                      );
                                  const absoluteIndex =
                                    globalIssuesCount +
                                    previousTablesIssuesCount +
                                    issueIndex;
                                    ...
                                    >
                                      <div className="flex items-start">
                                        <input
                                          ...
                                          onChange={() =>
                                            toggleIssueSelection(absoluteIndex)
                                          }
                                          className="mr-3 mt-1"
                                        />
                                        <div className="flex-1">
            ...
            <CardFooter>
              ...
              >
                ...
                {isFixing
                  ? "Fixing Issues..."
                  : `Fix Selected Issues (${selectedIssues.length})`}
              </Button>
            </CardFooter>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="custom" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Write custom SQL to fix database schema issues
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Textarea
...
CHECK (role IN ('recruit', 'volunteer', 'admin'));"
            ...
            <CardFooter>
              <Button
                ...
                disabled={isFixing || !customSql.trim()}
                className="w-full"
              >
