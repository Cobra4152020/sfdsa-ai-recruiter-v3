...
export function LoginAuditDashboard({ events, onRefresh }: LoginAuditDashboardProps) {
  ...
  const failedAttempts = events.filter((event) => event.eventType === "failed_attempt")
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-lg font-bold text-[#0A3C1F]">Login Activity</CardTitle>
            <CardDescription>Recent login events and security alerts</CardDescription>
          </div>
          {onRefresh && (
      ...
      <CardContent>
        {hasRecentFailedAttempts && (
          <Alert variant="destructive" className="mb-4">
            ...
            <AlertDescription>
              {failedAttempts.length} failed login attempt{failedAttempts.length > 1 ? "s" : ""} detected. Please review the
              activity below.
            </AlertDescription>
          </Alert>
        )}
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{event.userAgent}</p>
                  </div>
                </div>
