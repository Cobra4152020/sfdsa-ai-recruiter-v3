...
import {
  ...
  generateSimplifiedMermaidDiagram,
} from "@/lib/schema-visualization";
...
export function SchemaVisualization({ schema }: SchemaVisualizationProps) {
  const [diagramType, setDiagramType] = useState<"detailed" | "simplified">(
    "simplified",
  );
  const [zoomLevel, setZoomLevel] = useState(100);

  const getMermaidDiagram = () => {
    return diagramType === "detailed"
      ? generateMermaidDiagram(schema)
      : generateSimplifiedMermaidDiagram(schema);
  };
      ...
      <CardContent>
        <div className="mb-4 flex items-center justify-between">
          <Tabs
            value={diagramType}
            onValueChange={(value) =>
              setDiagramType(value as "detailed" | "simplified")
            }
          >
            <TabsList>
