  ...
  return (
    <Card className="w-full max-w-3xl mx-auto">
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            <div className="p-3 bg-green-50 text-green-800 rounded-md flex items-center">
              ...
              <div>
                <p className="font-semibold">Successfully fetched {questions.length} questions</p>
              ...
              <div key={i} className="border rounded-md p-4">
                ...
                <div className="grid gap-2 mb-3">
                  {q.options.map((option: string, j: number) => (
                    ...
                    >
                      <span className="inline-block w-6">{String.fromCharCode(65 + j)}.</span>
                      {option}
                      {j === q.correctAnswer && <CheckCircle className="h-4 w-4 inline ml-2 text-green-600" />}
                    </div>
                  ))}
                </div>
      ...
      <CardFooter className="text-sm text-gray-500 border-t pt-4">
        This component tests if the trivia questions API is working correctly with fallback questions.
      </CardFooter>
    </Card>
  )
}
