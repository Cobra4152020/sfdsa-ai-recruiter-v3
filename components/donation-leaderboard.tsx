...
export function DonationLeaderboard({ limit = 5, showViewAll = true }: DonationLeaderboardProps) {
  ...
  const fetchLeaderboard = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/donations/leaderboard?limit=${limit}&offset=${page * limit}`)
          ...
          <>
            <div className="space-y-4">
              {leaderboard.length === 0 ? (
                <p className="text-center py-4 text-muted-foreground">No donation data available yet.</p>
              ) : (
                leaderboard.map((entry) => (
                  <div key={entry.user_id} className="flex items-center space-x-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        {entry.last_donation_date && ` • Last: ${formatDate(entry.last_donation_date)}`}
                      </p>
                    </div>
