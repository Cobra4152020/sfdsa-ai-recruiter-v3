...
export function RecruiterPerformanceChart({
  ...
  const calculateTrend = () => {
    ...
    const currentValue = data[data.length - 1][
      metricType as keyof PerformanceData
    ] as number;
      ...
      <CardHeader>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
          <div>
            ...
            <CardDescription>
              Track your recruitment metrics over time
            </CardDescription>
          </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            {chartType === "line" ? (
              ...
              <BarChart data={data}>
                ...
                <Legend />
                {metricType === "referrals" && (
                  <Bar dataKey="referrals" fill="#0A3C1F" name="Referrals" />
                )}
                {metricType === "hires" && (
                  <Bar dataKey="hires" fill="#FFD700" name="Hires" />
                )}
                {metricType === "clicks" && (
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                <div>
                  ...
                  <p className="text-lg font-medium">
                    {data[data.length - 1].referrals}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Hires</p>
                  <p className="text-lg font-medium">
                    {data[data.length - 1].hires}
                  </p>
                </div>
                <div>
            ...
            <div className="p-4 bg-muted border rounded-lg">
              ...
              <div className="mt-2">
                <p className="text-sm text-gray-600">
                  Over the past {data.length} months, you&apos;ve generated a
                  total of {data.reduce((sum, item) => sum + item.referrals, 0)}{" "}
                  referrals and{" "}
                  {data.reduce((sum, item) => sum + item.hires, 0)} hires.
                </p>
                <p className="text-sm text-gray-600 mt-2">
                  Your average monthly performance:
                </p>
                <ul className="mt-1 text-sm text-gray-600 space-y-1">
                  <li>
                    •{" "}
                    {(
                      data.reduce((sum, item) => sum + item.referrals, 0) /
                      data.length
                    ).toFixed(1)}{" "}
                    referrals
                  </li>
