        ...
        <CardContent className="p-6">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Badge variant={gameStats.currentStreak >= 3 ? "default" : "outline"} className="text-sm">
              ...
              {currentQuestion && (
                <Badge variant="outline" className="text-sm">
                  {currentQuestion.difficulty.toUpperCase()}
                  {gameStats.difficultyMultiplier > 1 && ` (${gameStats.difficultyMultiplier}x)`}
                </Badge>
              )}
            </div>
      ...
      {mode === "study" && isGameOver && reviewCards.length > 0 && (
        <Card className="mt-6">
          <CardHeader>
            ...
            <CardDescription>
              You had trouble with {reviewCards.length} question{reviewCards.length > 1 ? "s" : ""}. Take some time to
              review them.
            </CardDescription>
          </CardHeader>
          <CardContent>
