...
export function BadgeAnalytics({ analytics, className }: BadgeAnalyticsProps) {
  ...
  const averageCompletionRate = useMemo(() => {
    const total = analytics.reduce((sum, badge) => sum + badge.completionRate, 0)
          ...
          <div className="h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData}>
                ...
                <Tooltip />
                <Bar yAxisId="left" dataKey="earned" fill="#8884d8" name="Total Earned" />
                <Bar yAxisId="right" dataKey="rate" fill="#82ca9d" name="Completion Rate (%)" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
